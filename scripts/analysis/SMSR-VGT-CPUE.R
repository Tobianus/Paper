...
allmeans_nage_noscale <- allmeans_nage %>%
  mutate(Model = 'MODEL')

cpue_by_year_noscale <- cpue_by_year %>%
  mutate(Model = 'OBSERVED')

CAGE_CPUE <- rbind(allmeans_nage_noscale, cpue_by_year_noscale) 
...
p <- ggplot(CAGE_CPUE, aes(x = Year, y = total_cpue, color = group, linetype = Model)) +
  geom_line(linewidth = 1.5) +
  labs(title = "Unscaled Total CPUE Over Time by Groups",
       x = "Year",
       y = "CPUE in 1000 t",
       color = "Group",
       linetype = "Model") +
