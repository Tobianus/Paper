...
ggsave("Facet - Center of Gravity for SA 1 (2010-2024).png", plot = p, dpi = 600, width = 6, height = 5)
...
p <- ggplot() +
  ...
  # Plot center of gravity points for each year and area
  #geom_point(data = catch_by_year, aes(x = lon, y = lat, color = group, size = total_catch), alpha = 0.4) +  # Plot catch points with size based on total catch
  geom_point(data = cog_per_year_area_vgt, aes(x = lon_cog, y = lat_cog, fill = area), size = 3) +
  # Add text labels for years (optional, not in legend)
  geom_text_repel(data = cog_per_year_area_vgt, aes(x = lon_cog, y = lat_cog, label = Year), size = 2, show.legend = FALSE,
            max.overlaps = 10000, box.padding = 0.25, point.padding = 0.2, segment.size = 0.3) +
  #geom_density_2d(data = catch_by_year, aes(x = lon, y = lat), color = "black", size = 0.3) +
  #geom_point(data = catch_by_year, aes(x = lon, y = lat, color = group, size = total_catch, shape = factor(Year))) +
...
ggsave("Center of Gravity for SA 1 (2010-2024).png", plot = p, dpi = 600, width = 5.5, height = 6)
